...
# Same augmentation pipeline as the AlexNet baseline, but cropped to 224
# instead of 227 since that is the input size ResNet expects.
# Resize to 256 first so the random crop can sample different regions each epoch.
...
# Batch size and worker count match the AlexNet run to keep the comparison fair.
...
# from ImageNet, so the comparison against AlexNet is like for like.
model = models.resnet34(weights=None)
# Replace the 1000-class ImageNet head with one sized for Food-101.
# 512 is the feature width coming out of the final residual block in ResNet-34.
